...
def multihead_self_attention(
    ...
    K=einsum(k_proj_weight,in_features,"... dk d_in, ... seq_len d_in -> ... seq_len dk")
    K=rearrange(K,"... seq_len (num_heads dk_head) -> ...  num_heads seq_len dk_head",num_heads=num_heads)
    
    Q=einsum(q_proj_weight,in_features,"... dk d_in, ... seq_len d_in -> ... seq_len dk")
    Q=rearrange(Q,"... seq_len (num_heads dk_head) -> ...  num_heads seq_len dk_head",num_heads=num_heads)
    
    if token_positions is not None:
        rpe=RotaryPositionalEmbedding(theta,d_model/num_heads,seq_len)
        Q=rpe(Q,token_positions)
        K=rpe(K,token_positions)
       
        
    
    V=einsum(v_proj_weight,in_features,"... dv d_in, ... seq_len d_in -> ... seq_len dv")
    ...
    multihead_attention=einsum(o_proj_weight,attention_scores,"... d_model dv, ... seq_len dv -> ... seq_len d_model")
